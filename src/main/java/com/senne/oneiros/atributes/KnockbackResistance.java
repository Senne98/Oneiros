package com.senne.oneiros.atributes;

import com.senne.oneiros.Oneiros;
import com.senne.oneiros.atributes.attributeTypes.EquipmentDoubleAttribute;
import org.bukkit.Material;
import org.bukkit.NamespacedKey;
import org.bukkit.attribute.Attribute;
import org.bukkit.inventory.ItemStack;

import java.util.ArrayList;

import static com.senne.oneiros.tools.utils.SerializationUtils.serialize;

public class KnockbackResistance extends EquipmentDoubleAttribute {

    public KnockbackResistance() {
        name = "Knockback Resistance";
        namespacedKey = new NamespacedKey(Oneiros.getPlugin(), "knockbackresistance");
        icon = new ItemStack(Material.NETHERITE_CHESTPLATE);
        attribute = Attribute.KNOCKBACK_RESISTANCE;
        min = -1;
        max = 1;
    }

    @Override
    public com.senne.oneiros.atributes.attributeTypes.Attribute copy() {
        KnockbackResistance clone = new KnockbackResistance();
        clone.setAmount(amount);
        clone.setSlots(new ArrayList<>(slots));
        return clone;
    }
}